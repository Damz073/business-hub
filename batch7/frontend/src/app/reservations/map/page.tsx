...
function ReservationsMapContent() {
  ...
  const blocks = useMemo(() => {
    return (data?.items || []).map((item) => {
      const startIndex = Math.max(0, days.findIndex((day) => day === (item.checkin || calendarStart)));
      const tone = statusToneMap[item.calendar_status || item.status || "lead"] || "is-pending";
      return {
        ...item,
        tone,
        startIndex,
        span: Math.min(diffDays(item.checkin || calendarStart, item.checkout || calendarEnd), days.length || 1),
      };
    });
  }, [data?.items, days, calendarStart, calendarEnd]);

  function matchesRoom(item: CalendarReservation, room: CalendarRoom) {
    const roomName = (room.name || "").trim().toLowerCase();
    const roomCode = (room.code || "").trim().toLowerCase();
    const roomCategory = (room.category || "").trim().toLowerCase();
  ...
  return (
    <>
      <div className="page-grid reservations-map-page">
        <div className="card hero-panel fade-up premium-glow">
          <div className="hero-copy">
            ...
            <p>
              Visualize a estadia em blocos contínuos por tipologia, acompanhe o pagamento e abra o detalhe completo
              da reserva sem sair do calendário.
            </p>
          </div>
          ...
          <div className="toolbar-group">
            <label className="label compact">
              ...
              <input
                ...
                onChange={(e) => setRange((current) => ({ ...current, start_date: e.target.value }))}
              />
            </label>
            <label className="label compact">
              Fim
              <input
                ...
                onChange={(e) => setRange((current) => ({ ...current, end_date: e.target.value }))}
              />
            </label>
        ...
        <div className="card fade-up reservations-map-board">
          <div className="map-scroll">
            <div className="map-board" style={{ gridTemplateColumns: `220px repeat(${days.length || 1}, 100px)`, position: "relative" }}>
              ...
              {(data?.rooms || []).map((room) => {
                const roomBlocks = blocks.filter((item) => matchesRoom(item, room));

                return (
                  <div key={String(room.id)} className="map-row-group">
                    <div className="map-room-label">
                      <strong>{room.name || "Quarto"}</strong>
                      <span>{room.code || room.category || "-"}</span>
                    </div>

                    <div className="map-room-track" style={{ gridColumn: `2 / span ${days.length || 1}` }}>
                      <div className="map-track-grid" style={{ gridTemplateColumns: `repeat(${days.length || 1}, 100px)` }}>
                        {days.map((day) => (
                          <div key={`${room.id}-${day}`} className="map-cell" />
                        ))}
                      </div>
                      ...
                      {roomBlocks.map((item) => (
                        <button
                          key={String(item.id)}
                          type="button"
                          className={`reservation-chip ${(item as CalendarReservation & { tone?: string }).tone || "is-pending"}`}
                          style={{
                            left: `${((item as CalendarReservation & { startIndex?: number }).startIndex || 0) * 100 + 6}px`,
                            width: `${((item as CalendarReservation & { span?: number }).span || 1) * 100 - 12}px`,
                          }}
                          onClick={() => setSelected(item)}
                        >
                          <strong>
                            {item.main_guest_name || item.guest_name || item.reservation?.main_guest_name || "Reserva"}
                          </strong>
                          <span>
                            {item.reservation_code || `RES-${item.id}`} • {item.payment_status || "sem status"}
                          </span>
                        </button>
                      ))}
                    </div>
          ...
          <div className="reservation-sidepanel-backdrop" onClick={() => setSelected(null)}>
            <div className="reservation-sidepanel card" onClick={(event) => event.stopPropagation()}>
              <div className="section-row">
                <div>
                  ...
                  <h2 className="section-title">
                    {selected.main_guest_name || selected.guest_name || selected.reservation?.main_guest_name || "Reserva"}
                  </h2>
              ...
              <div className="detail-grid">
                ...
                <div className="detail-item">
                  <span>Valor</span>
                  <strong>{formatCurrency(Number(selected.total_value || selected.reservation?.total_value || 0))}</strong>
                </div>
                <div className="detail-item">
                  <span>Origem</span>
                  <strong>{selected.reservation?.source || "-"}</strong>
                </div>
                <div className="detail-item">
                  <span>Telefone</span>
                  <strong>{selected.phone || selected.reservation?.main_guest_phone || selected.reservation?.guest_phone || "-"}</strong>
                </div>
                <div className="detail-item">
              ...
              <div className="section-row">
                <h3 className="section-title">Hóspedes</h3>
              </div>
      ...
      <style jsx global>{`
        ...
        .map-row-group { display: contents; }
        .map-corner,
        .map-day-header,
        .map-room-label,
        ...
        .map-day-header { position: sticky; top: 0; z-index: 10; min-width: 100px; padding: 14px 8px; text-align: center; font-size: 12px; color: var(--muted); display: grid; gap: 4px; background: var(--card); }
        .map-day-header strong { font-size: 15px; color: var(--foreground); }
        .map-room-label { position: sticky; left: 0; z-index: 11; width: 220px; min-width: 220px; max-width: 220px; background: var(--card); border-right: 1px solid rgba(148,163,184,.12); padding: 16px; display: grid; gap: 5px; align-content: center; }
        ...
        .reservation-chip { position: absolute; top: 14px; height: 58px; border-radius: 14px; padding: 10px 12px; display: grid; align-content: center; text-align: left; gap: 3px; border: 1px solid rgba(0,0,0,.08); box-shadow: 0 6px 14px rgba(0,0,0,.08); cursor: pointer; transition: transform .18s ease, box-shadow .18s ease, opacity .18s ease; overflow: hidden; }
        ...
        .reservation-chip.is-confirmed { background: #dcfce7; color: #166534; border-color: rgba(22,101,52,.16); }
        .reservation-chip.is-pending { background: #fef3c7; color: #92400e; border-color: rgba(146,64,14,.16); }
        .reservation-chip.is-problem { background: #fee2e2; color: #991b1b; border-color: rgba(153,27,27,.16); }
        .reservation-chip.is-blocked { background: #e5e7eb; color: #374151; border-color: rgba(55,65,81,.14); }
        [data-theme='dark'] .map-day-header { background: rgba(8,17,29,.98); }
        [data-theme='dark'] .map-room-label { background: rgba(8,17,29,.98); }
        [data-theme='dark'] .reservation-chip.is-confirmed { background: linear-gradient(135deg, rgba(16,185,129,.28), rgba(5,150,105,.2)); color: #d1fae5; border-color: rgba(16,185,129,.35); }
        [data-theme='dark'] .reservation-chip.is-pending { background: linear-gradient(135deg, rgba(245,158,11,.26), rgba(217,119,6,.18)); color: #fef3c7; border-color: rgba(245,158,11,.35); }
        [data-theme='dark'] .reservation-chip.is-problem { background: linear-gradient(135deg, rgba(239,68,68,.24), rgba(190,24,93,.18)); color: #fee2e2; border-color: rgba(239,68,68,.35); }
        [data-theme='dark'] .reservation-chip.is-blocked { background: linear-gradient(135deg, rgba(100,116,139,.28), rgba(51,65,85,.22)); color: #e2e8f0; border-color: rgba(148,163,184,.24); }
